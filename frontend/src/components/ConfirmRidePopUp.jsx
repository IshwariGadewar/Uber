...
const ConfirmRidePopUp = (props) => {
  ...
  return (
    <div>
        <h5 onClick={()=>{props.setConfirmRidePopUpPanel(false)}} className='absolute top-3 right-6 text-3xl'><i className="ri-arrow-down-wide-line"></i></h5>
        ...
        <div className='-my-2 flex flex-col justify-between items-center gap-2'>
            ...
            <div>
                <form onSubmit={(e)=>{submitHandler(e)}}>
                    <input value={otp} onChange={(e)=>{setOTP(e.target.value)}} className='bg-[#eee] px-8 py-2 text-lg font-mono rounded-lg w-full mt-5' type="number" placeholder='Enter OTP'/>
                    <Link to='/CaptainRidingToUser' className='w-full bg-yellow-400 flex justify-center text-white font-semibold p-2 rounded-lg mt-2'>Go To PickUp</Link>
                </form>
            </div>
        </div>
    </div>
